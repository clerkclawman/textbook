...
function generateNewsContent(date, rawData) {
  ...
  for (let i = 0; i < 50; i++) {
    ...
    const jpQuestion = translateToJapanese(question);
    
    // TWO-LINE FORMAT:
    // Line 1: Number. Emoji English headline - Discussion question?
    // Line 2: Japanese translation (NO English, NO emoji, NO leading space)
    content += `${i + 1}. ${emoji} ${headline} - ${question}\n${jpHeadline} - ${jpQuestion}についてどう思いますか？`;
...
console.log('Files generated successfully with correct TWO-LINE format!');
